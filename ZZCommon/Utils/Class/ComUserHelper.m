...
+ (void)login:(NSString *)username password:(NSString *)password success:(void (^)(UserInfo * _Nullable))success failure:(void (^)(ResponseModel * _Nullable))failure {
    [[[ZZApiHelper new] loginWithUsername:username password:password] startWithSuccess:^(YTKBaseRequest * _Nonnull request, LoginModel * _Nullable loginModel) {
...
+ (void)enter:(NSString *)accessToken success:(nullable void (^)(UserInfo * _Nullable))success failure:(nullable void (^)(ResponseModel * _Nullable))failure {
    ...
    [[[ZZApiHelper new] getBasicInfoWhenEnter] startWithSuccess:^(YTKBaseRequest * _Nonnull request, UserInfo * _Nullable responseModel) {
...
+ (void)logoutWithClear:(BOOL)clear {
    ...
    if (userHelper.accessToken) {
        if (clear) {
            // 退出登录
            [[[ZZApiHelper new] logout] startWithSuccess:nil failure:nil];
        }
...
+ (void)changeRootToLogin {
    ...
    [(ZZTabBarViewController *)(window.rootViewController) changeRootToLogin];
}
