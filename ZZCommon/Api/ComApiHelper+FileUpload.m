...
@implementation ComApiHelper (FileUpload)
...
+ (void)uploadWithFileDatas:(NSArray<ComUploadFile *> *)fileDatas batchRequestTask:(void (^)(YTKBatchRequest * _Nullable))batchRequestTask success:(void (^)(YTKBatchRequest * _Nullable, NSArray<NSString *> * _Nullable))success failure:(void (^)(YTKBatchRequest * _Nullable, ResponseModel * _Nullable))failure {
    ...
    [ComApiHelper getBatchTasks:fileDatas block:^(NSArray<ComApiHelper *> *tasks) {
        ...
        [batchRequest startWithCompletionBlockWithSuccess:^(YTKBatchRequest *batchRequest) {
            ...
            if (failure) {
                ResponseModel *responseModel = [ComApiHelper failureResponseModel:batchRequest.failedRequest];
...
/**
 获取批量任务
 */
+ (void)getBatchTasks:(NSArray<ComUploadFile *> *)fileDatas block:(void(^)(NSArray<ComApiHelper *> *tasks))block {
    NSMutableArray<ComApiHelper *> *tasks = [NSMutableArray arrayWithCapacity:fileDatas.count];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_semaphore_t sema = dispatch_semaphore_create(0);
        for (int i=0;i<fileDatas.count;i++) {
            ComUploadFile *obj = fileDatas[i];
            if (obj.fileData != nil && obj.fileUrl == nil) {
                ComApiHelper *api = [ComApiHelper new];
                ...
                [obj.asset requestOriginImageWithCompletion:^(UIImage *result, NSDictionary<NSString *,id> *info) {
                    if (result) {
                        ...
                        NSString *fileName = [ComApiHelper getFileName];
                        ComApiHelper *api = [ComApiHelper new];
