...
- (NSDictionary<NSString *,NSString *> *)requestHeaderFieldValueDictionary {
    return [ComApiHelper handleHeaders:_requestHeaderFieldValueDictionary];
}
...
- (void)startWithSuccess:(void (^)(YTKBaseRequest * _Nonnull, NSObject * _Nullable))success failure:(void (^)(YTKBaseRequest * _Nonnull, ResponseModel * _Nullable))failure {
    ...
    [self startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        RACTuple *responseModel = [ComApiHelper successResponseModel:request];
        ...
        if (failure) {
            ResponseModel *responseModel = [ComApiHelper failureResponseModel:request];
...
+ (ResponseModel *)failureResponseModel:(YTKRequest *)failureRequest {
    ...
    if (!responseModel) {
        responseModel = [[ResponseModel alloc] initWithCode:failureRequest.responseStatusCode msg:[ComApiHelper handleResponseMsg:failureRequest.error.code]];
    }
...
- (BOOL)statusCodeValidator {
    ...
    if ((statusCode == 401 || statusCode == 424) && ![_requestUrl isEqualToString:@"auth/token/logout"]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (![[QMUIHelper visibleViewController] isKindOfClass:[QDBaseLoginViewController class]]) {
